...
namespace Maticsoft.DAL.NewsManage
{
    public class News
    {
        ...
        /// 增加一条数据
        /// </summary>
        public DataSet  Add(Maticsoft.Model.NewsManage.News model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into T_News(");
            strSql.Append("UserId,Heading,Content,Frequency,IssueDate,Dormancy,ClassId,Focus,Priority,IsTop,IsLimited)");
        ...
        ///// 增加一条数据
        ///// </summary>
        //public void AddNewsImages(Maticsoft.Model.NewsManage.News model)
        //{
        //    StringBuilder strSql = new StringBuilder();
        ...
        public Maticsoft.Model.NewsManage.News GetModel(int NewsId)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["IsLimited"].ToString() != "")
                {
                    model.IsLimited = int.Parse(ds.Tables[0].Rows[0]["IsLimited"].ToString());
                }
                #endregion
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select [NewsId],[UserId],[Heading],[Content],[Frequency],[IssueDate],[Dormancy],[ClassId],[ClassDesc],[Focus],[Priority],[IsTop] ");
            strSql.Append(" FROM T_News_view ");
            if (strWhere.Trim() != "")
            {
		...
		public DataSet GetNewsList(int top,string strWhere,string FiledOrder)
		{
            ...
            strSql.Append(" [NewsId],[UserId],[Heading],[Content],[Frequency],[IssueDate],[Dormancy],[ClassId],[ClassDesc],[Focus],[Priority],[IsTop]  ");
            strSql.Append(" from T_News_View ");			
        ...
        //读取图片新字
        public DataSet GetNewsImgList(int top, string strWhere, string FiledOrder)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ");
            if (top > 0)
            ...
            strSql.Append(" * ");
            strSql.Append(" from T_Newsimages_View ");
        ...
        public DataSet GetNewsList(int top, int ClassId, bool Dormancy)
        {
            ...
            return GetNewsList(top, strSql.ToString(), "issuedate desc,NewsId desc");
        }
        ...
        /// <param name="FiledOrder">排序字段</param>
        /// <returns></returns>
        public DataSet GetNewsImagesList(int top, string strWhere, string FiledOrder)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ");
            if (top > 0)
        ...
        public DataSet GetTopImgScroll(int top)
        {
            return GetNewsImgList(top, "", " newsid desc,newsimagesid ");
        }
