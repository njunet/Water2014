<!--
/* 调用方法：
  <asp:TextBox ID="txtApplyDate" runat="server" CssClass="txtInput" onfocus="calendar()">2007-10-26</asp:TextBox><img src="../Image/calendar.gif" id="imgApply" alt="Please select Date" style="cursor:hand;" onclick="calendar('txtApplyDate')" />
*/
var cal_Width = 200;
...
{
    this.regInfo    = "WEB Calendar ver 3.0&#13;Close the Calendar keyboard shortcuts：[Esc]";
...
function writeIframe()
{
    ...
    "    <td width='10%' height='19px' class='bg' title='Previous 1 Month&#13;keyboard shortcuts：←' style='cursor: hand' onclick='parent.prevM()'><b>&lt;</b></td>"+
...
//    for (var i=yearMin; i<=yearMax; i++) strIframe += "<option value='"+i+"'>"+i+"年</option>";
    ...
    "</td><td width='15%' bgcolor='#95B7F3'>年</td>"+
    ...
    strIframe += "</select>"+
    "</td><td width='10%' bgcolor='#95B7F3'>月</td>"+
    "    <td width='10%' class=bg title='Next 1 Month&#13;keyboard shortcuts：→' onclick='parent.nextM()' style='cursor: hand'><b>&gt;</b></td></tr></table>"+
    ...
    "    <tr align=center><td height='20px'>Sun</td><td>Mon</td><td>Tur</td><td>Wed</td><td>Thu</td><td>Fri</td><td>Sat</td></tr></table>"+
    ...
    "    <table id=tableDay height='120px' width='"+ cal_Width +"px' border=0 cellspacing=1 cellpadding=0>";
         ...
         " type=button value='清除' onclick='parent.document.all[parent.WebCalendar.objExport].value=\"\";parent.hiddenCalendar()'>" + 
         "&nbsp;" + //alert(parent.WebCalendar.objExport)
    ...
    "    <tr><td><input name=prevYear title='Previous 1 Year&#13;keyboard shortcuts：↑' onclick='parent.prevY()' type=button value='&lt;&lt;'"+
    "    onfocus='this.blur()' style='meizz:expression(this.disabled=parent.WebCalendar.thisYear==1000)'><input"+
    "    onfocus='this.blur()' name=prevMonth title='Previous 1 Month&#13;keyboard shortcuts：←' onclick='parent.prevM()' type=button value='&lt;&nbsp;'>"+
    "    </td><td align=center><input name=today type=button value='今天' onfocus='this.blur()' style='width: 50px;' title='Today&#13;keyboard shortcuts：T'"+
    "    onclick=\"parent.returnDate(new Date().getDate() +'/'+ (new Date().getMonth() +1) +'/'+ new Date().getFullYear())\">"+
    "    </td><td align=right><input title='Next 1 Month&#13;keyboard shortcuts：→' name=nextMonth onclick='parent.nextM()' type=button value='&nbsp;&gt;'"+
    "    onfocus='this.blur()'><input name=nextYear title='Next 1 Year&#13;keyboard shortcuts：↓' onclick='parent.nextY()' type=button value='&gt;&gt;'"+
...
function calendar(txtID) //主调函数
{
    ...
    try
	{
		...
		{
            WebCalendar.dateStyle = document.all(WebCalendar.objExport).value.match(WebCalendar.dateReg);
            if (WebCalendar.dateStyle == null)
            {
                WebCalendar.thisYear   = new Date().getFullYear();
                WebCalendar.thisMonth  = new Date().getMonth()+ 1;
                WebCalendar.thisDay    = new Date().getDate();
                //alert("Date format invalid！\nPlease check the minute and second format！");
...
function writeCalendar() //对日历显示的数据的处理程序
{
    ...
    if (!(y<=9999 && y >= 1000 && parseInt(m, 10)>0 && parseInt(m, 10)<13 && parseInt(d, 10)>0)){
        alert("Sorry，You Date format is invalid！");
        WebCalendar.thisYear   = new Date().getFullYear();
        WebCalendar.thisMonth  = new Date().getMonth()+ 1;
        WebCalendar.thisDay    = new Date().getDate(); }
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport != "")
    {
        ...
        if(d==null){alert("Date format is invalid！\r\n\r\nPlease reset WebCalendar.format ！"); return false;}
		...
		//最为关键的一个修改//document.all[WebCalendar.objExport].value = returnValue;
        hiddenCalendar();
    }
}
//-->